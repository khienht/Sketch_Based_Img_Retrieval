...
photo_data = pickle.load(open('photo-vgg-0epoch.pkl', 'rb'))
sketch_data = pickle.load(open('sketch-vgg-0epoch.pkl', 'rb'))
print(photo_data['name'])
...
# print(photo_feature[0].shape)
# print('size:', np.size(photo_feature, 0))
...
for ii, (query_sketch, query_name) in tqdm.tqdm(enumerate(zip(sketch_feature, sketch_name))):
    ...
    for i in indices[0]:
        ...
        retrievaled_name = retrievaled_name.split('/')[1]
        retrievaled_name = retrievaled_name.split('.')[0]
        print('retriev:',retrievaled_name)
